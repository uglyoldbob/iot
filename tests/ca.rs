...
async fn ssh_genkey() {
    ...
    for method in methods {
        ...
        if let (Ok(public_key_openssh), Ok(private_key_openssh)) = (
            ...
            let russh_private_result =
                russh::keys::PrivateKey::from_openssh(private_key_openssh.as_bytes());
            ...
            {
                let russh_private_key = russh::keys::key::PrivateKeyWithHashAlg::new(
                    Arc::new(russh_private_key),
                    Some(HashAlg::Sha256),
                );
                ...
                let server_russh_key =
                    russh::keys::PrivateKey::from_openssh(server_private_openssh.as_bytes())
                        .expect("Failed to convert ssh server key");
        ...
        server.run_with_shutdown(true);
        {
            ...
            loop {
                ...
                let d = tokio::time::timeout(
                    ...
                    c.get("http://127.0.0.1:3000").send(),
                )
                .await;
                match d {
                    Ok(Ok(t)) => {
...
async fn existing_password() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[cfg(feature = "tpm2")]
    a.stderr(predicate::str::contains("Password file already exists"));
    #[cfg(not(feature = "tpm2"))]
    a.stderr(predicate::str::contains("Credentials file already exists"));
    Ok(())
}
